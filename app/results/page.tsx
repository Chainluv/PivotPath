...
export default function ResultsPage({ searchParams }: ResultsPageProps) {
  const currentRole = searchParams?.currentRole || "your current field";
  const targetInterest = searchParams?.targetInterest || "tech";
  const educationLevel = searchParams?.educationLevel || "not specified";
  const location = searchParams?.location || "your area";
  const goal = searchParams?.goal || "switch careers";

  const careers = mockCareerData[targetInterest] || mockCareerData.tech;
